...
description: "Pre-landing code review — activate when user asks to review a diff or check code before merging"
---
...
### Batch ASK items as decision briefs.

For each ASK item, format as a decision brief. Number them D1, D2, D3 across the whole review:

```
I auto-fixed N issues. M need your input.

D1: <one-line title> (file:line)
...
Recommendation: A) Fix because <one-line reason>
Completeness: A=10/10, B=3/10  (or: Note: options differ in kind, not coverage — no completeness score)
...
Rules:
- **Recommendation is ALWAYS present.** Even for taste calls: `Recommendation: <default> — taste call, no strong preference`. Keep `(recommended)` on one option.
- **Completeness scores ONLY when options differ in coverage** (Fix vs Skip is coverage-differentiated). When options differ in kind (Fix approach A vs Fix approach B), write the kind-note instead. Do not fabricate scores.
